...
class _QrManagementPanelState extends ConsumerState<QrManagementPanel> {
  ...
  Future<void> _regenerate(BuildContext context) async {
    ...
    try {
      await ref
          .read(qrManagementRepositoryProvider)
          .regenerateLocalMetadata(
            restaurantId: widget.restaurantId,
            branchId: widget.branchId,
          );
      if (!context.mounted) return;
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('QR metadata regenerated')));
    } finally {
  ...
  Widget build(BuildContext context) {
    return Wrap(
      ...
      children: [
        ...
        _QrActionButton(
          tooltip: 'Regenerate QR metadata',
          icon: regenerating ? Icons.hourglass_top : Icons.refresh,
          label: 'Regenerate',
