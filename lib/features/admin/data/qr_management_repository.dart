...
class QrManagementRepository {
  ...
  Future<void> regenerateLocalMetadata({
    required String restaurantId,
    required String branchId,
  }) async {
    final ref = _firestore.doc(FirestorePaths.branch(restaurantId, branchId));
    await _firestore.runTransaction((transaction) async {
      ...
      final info = _fromBranchData(
        restaurantId: restaurantId,
        branchId: snapshot.id,
        data: data,
      );
      final nextVersion = info.qrVersion + 1;
      transaction.update(ref, {
        'branchSlug': info.branchSlug,
        'queueUrl': info.queueUrl,
        'qrSlug': info.qrSlug,
        'qrImageUrl': info.pngAssetPath,
        'qrPngUrl': info.pngAssetPath,
        'qrSvgUrl': info.svgAssetPath,
        'qrPngLocalPath': info.pngAssetPath,
        'qrSvgLocalPath': info.svgAssetPath,
        'qrGeneratedAt': FieldValue.serverTimestamp(),
        'qrVersion': nextVersion,
        'qrAssetMode': 'local',
        'qrCode': {
          'queueUrl': info.queueUrl,
          'pngUrl': info.pngAssetPath,
          'svgUrl': info.svgAssetPath,
          'pngLocalPath': info.pngAssetPath,
          'svgLocalPath': info.svgAssetPath,
          'version': nextVersion,
          'assetMode': 'local',
        },
      });
